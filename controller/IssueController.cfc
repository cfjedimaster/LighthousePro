		...
		<cfloop query="projects">
			...
			<cfloop query="mail">
				...
				<cfif findNoCase(me, to)>
					...
					<cfset project = beans.projectService.getProject(projectid)>
					<cfset issue = beans.issueService.getIssue()>
					<cfset issue.setProjectIDFK(projectid)>
					<cfset issue.setName("MAIL ISSUE: " & subject)>
					<cfset history = "Created by email from #from# : #dateFormat(now(),variables.dateformat)# #timeFormat(now(),'h:mm tt')#">
					<cfset issue.setHistory(history)>
					<cfset issue.setDescription(body)>
					<cfset issue.setCreated(now())>
					<cfset issue.setUpdated(now())>
					
					<!--- four possible defaults --->
					<cfif project.getDefaultSeverity() neq "">
						<cfset issue.setSeverityIdFk(project.getDefaultSeverity())>
					</cfif>
					<cfif project.getDefaultStatus() neq "">
						<cfset issue.setStatusIdFk(project.getDefaultStatus())>
					</cfif>
					<cfif project.getDefaultIssueType() neq "">
						<cfset issue.setIssueTypeIdFk(project.getDefaultIssueType())>
					</cfif>
...
Due Date: <cfif len(issueBean.getDueDate())>#dateFormat(issueBean.getDueDate(), variables.dateformat)#</cfif><cfif isDefined("arguments.oldBean") and len(oldduedate)> was #dateFormat(oldduedate, "m/d/yy")#</cfif>
...
Created: #dateFormat(issueBean.getCreated(),variables.dateformat)# #timeFormat(issueBean.getCreated(),"h:mm tt")#

Updated:#dateFormat(issueBean.getUpdated(),variables.dateformat)# #timeFormat(issueBean.getUpdated(),"h:mm tt")#
...
<style type="text/css">
       ...
       <cfset style.body="font-family:verdana, helvetica, arial, sans-serif;margin:0;padding:0;">
       <cfset style.h1="font-family:verdana, helvetica, arial, sans-serif;margin:0;padding:0;">
       <cfset style.h1span="color:##fff;">
       <cfset style.dl="border-top:15px solid ##000;padding:1em;background:##E6DFCF;overflow:auto;margin:0;">
       <cfset style.dt="float:left;margin:.5em 0 0 0;font-size:small;clear:left;width:15%;font-weight:bold;text-align:right;padding-right:.5em;">
       <cfset style.dd="float:left;margin:.5em 0 0 0;font-size:small;width:80%;">
       <cfset style.pfootnote="clear:both; margin:0;font-size:small;padding:.5em;">

</head>

<body style="#style.body#">
<h1 style="#style.h1#">Lighthouse <span>Pro</span> Notification</h1>
<dl style="#style.dl#">
       <dt style="#style.dt#">Project:</dt>
       <dd style="#style.dd#">#project.getname()#</dd>
       <dt style="#style.dt#">Issue:</dt>
       <dd style="#style.dd#">#issueBean.getPublicID()#) #issueBean.getName()# <cfif isDefined("arguments.oldBean") and issueBean.getName() neq arguments.oldBean.getName()> (was #arguments.oldBean.getName()#)</cfif></dd>
       <dt style="#style.dt#">Project Area:</dt>
       <dd style="#style.dd#"><cfif isObject(locus)>#locus.getName()#<cfelse>#locus#</cfif> <cfif isObject(oldLocus) and isObject(locus) and locus.getName() neq oldLocus.getName()> (was #oldLocus.getName()#)</cfif></dd>
       <dt style="#style.dt#">Milestone:</dt>
       <dd style="#style.dd#"><cfif len(issueBean.getMilestoneIDFK())>#milestone#</cfif> <cfif isDefined("arguments.oldBean") and milestone neq oldmilestone and oldmilestone neq ""> (was #oldmilestone#)</cfif></dd>
       <dt style="#style.dt#">Severity:</dt>
       <dd style="#style.dd#">#severity# <cfif isDefined("arguments.oldBean") and severity neq oldseverity and oldseverity neq ""> (was #oldseverity#)</cfif></dd>
       <dt style="#style.dt#">Status:</dt>
       <dd style="#style.dd#">#status# <cfif isDefined("arguments.oldBean") and status neq oldstatus and oldstatus neq ""> (was #oldstatus#)</cfif></dd>
       <dt style="#style.dt#">Due Date:</dt>
       <dd style="#style.dd#"><cfif len(issueBean.getDueDate())>#dateFormat(issueBean.getDueDate(), variables.dateformat)#</cfif><cfif isDefined("arguments.oldBean") and len(oldduedate)> was #dateFormat(oldduedate, "m/d/yy")#</cfif></dd>
       <dt style="#style.dt#">Type:</dt>
       <dd style="#style.dd#">#issuetype# <cfif isDefined("arguments.oldBean") and issuetype neq oldissuetype and oldissuetype neq ""> (was #oldissuetype#)</cfif></dd>
       <dt style="#style.dt#">Related URL:</dt>
       <dd style="#style.dd#"><cfif len(issueBean.getRelatedURL())><a href="#issueBean.getRelatedURL()#">#issueBean.getRelatedURL()#</a></cfif></dd>
       <dt style="#style.dt#">Creator:</dt>
       <dd style="#style.dd#"><cfif isObject(creator) and len(creator.getName())>#creator.getName()#</cfif></dd>
       <dt style="#style.dt#">Created:</dt>
       <dd style="#style.dd#">#dateFormat(issueBean.getCreated(),variables.dateformat)# #timeFormat(issueBean.getCreated(),"h:mm tt")#</dd>
       <dt style="#style.dt#">Updated:</dt>
       <dd style="#style.dd#">#dateFormat(issueBean.getUpdated(),variables.dateformat)# #timeFormat(issueBean.getUpdated(),"h:mm tt")#</dd>
       <dt style="#style.dt#">Owner:</dt>
       <dd style="#style.dd#"><cfif isObject(user)>#user.getName()# <cfif isObject(oldUser) and user.getName() neq oldUser.getName()> (was #oldUser.getName()#)</cfif></cfif></dd>
       <dt style="#style.dt#">Description:</dt>
       <dd style="#style.dd#">#helpers.udfs.paragraphFormat2(htmlEditFormat(issueBean.getDescription()))#</dd>
       <dt style="#style.dt#">History:</dt>
       <dd style="#style.dd#">#helpers.udfs.paragraphFormat2(htmlEditFormat(issueBean.getHistory()))#</dd>
       <dt style="#style.dt#">Link:</dt>
       <dd style="#style.dd#"><a href="#link#">Log into Lighthouse to update issue</a></dd>
</dl>

<p style="##style.pfootnote##">This email was generated by Lighthouse Pro, an open-source ColdFusion bug tracker application created by Raymond Camden. Find more information at the Lighthouse Pro web site: <a href="http://lighthousepro.riaforge.org">http://lighthousepro.riaforge.org</a></p>
</body>
</html>
</cfoutput>
</cfsavecontent>
	...
	<cffunction name="getIssue" access="public" output="false">
		...
		<!--- also set defaults based on the project --->
		<cfif issue.getId() is 0>
			...
			<!--- No need to validate if the default is valid for the project - since you can't set it. Still feels messy though --->
			<cfif project.getDefaultLocus() neq "">
				<cfset issue.setLocusIdFk(project.getDefaultLocus())>
			</cfif>
	...
	<cffunction name="getIssues" access="public" output="false">
		...
		<cfset var archived = arguments.event.getValue("archived",false)>
		<cfset var issues = "">
		...
		<cfinvoke component="#beans.issueService#" method="getIssues" returnVariable="issues">
			...
			<cfinvokeargument name="archived" value="#archived#">
	...
	<cffunction name="saveIssue" access="public" output="false">
		...
		<cfset var archived = arguments.event.getValue("archived")>

		<cfif archived is "">
			<cfset archived = false>
			<cfset arguments.event.setValue("archived",false)>
		</cfif>
		...
		<cfset issue.setArchived(arguments.event.getValue("archived"))>		
		...
		<cfset issue.setArchived(arguments.event.getValue("archived"))>
		<cfset historyheader = "#dateFormat(now(),variables.dateformat)# #timeFormat(now(),'h:mm tt')# / #me.getUsername()# (#me.getName()#)#chr(10)#">
		...
		<cfelse>
			...
			<cfif issue.getArchived() neq oBean.getArchived()>
				<cfset history = history & "Archived changed from ""#oBean.getArchived()#"" to ""#issue.getArchived()#""#chr(10)#">
			</cfif>	
		...
		<cfif not arrayLen(errors)>
			...
			<cfif projectChangedFlag>
				...
				<cfif not me.hasRole("admin")>
					<cfif not listFindNoCase(valueList(myProjects.id), issue.getProjectIDFK())>
